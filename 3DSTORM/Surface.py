class surface(object):
    ...
    def evaluation(self):
        ...
        x = 0.5*np.matmul(np.linalg.inv(A), B)
        ...
        self.radius = np.sqrt(x[0]**2 + x[1]**2 + x[2]**2 + ((s_uu+s_vv+s_ww)/N))
        self.radius_i = np.sqrt((self.data.points["X [nm]"] - self.X_cent)**2 + (self.data.points["Y [nm]"] - self.Y_cent)**2 + (self.data.points["Z [nm]"] - self.Z_cent)**2)
